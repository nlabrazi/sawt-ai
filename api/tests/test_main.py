...
def test_parse_allowed_origins_normalizes_and_deduplicates_values():
    origins = parse_allowed_origins(
        " https://sawt-ai.nabster.dev/ , http://localhost:3000,https://sawt-ai.nabster.dev "
    )

    assert origins == [
        "https://sawt-ai.nabster.dev",
        "http://localhost:3000",
    ]


def test_build_cors_options_uses_explicit_origin_allowlist_without_regex():
    options = build_cors_options("https://sawt-ai.nabster.dev,http://localhost:3000")
